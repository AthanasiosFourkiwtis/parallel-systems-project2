...
- **Tiled με shared memory αντί naive.** Ξεκίνησα από μια naive έκδοση όπου
  ...
  ανά tile** και να επαναχρησιμοποιείται από όλα τα threads του block.
- **Block geometry — 2D με σταθερό πλάτος.** Tο block είναι `TILE_SIZE × BLOCK_ROWS`,
...
- **Φόρτωση του B με εσωτερικό loop.** Επειδή το tile του B είναι πάντα
  ...
  τις διαμορφώσεις.
- **`__syncthreads()` ανάμεσα σε φόρτωση και υπολογισμό**, και ένα δεύτερο
  πριν το επόμενο tile, για να μη χρησιμοποιεί κανένα thread παλιά shared
  data ενώ άλλο γράφει νέα.
- **Χρονομέτρηση δίκαιη.** Η σειριακή τρέχει 1 φορά (στο N=2048 αργεί
  ...
  όπως ζητάει η εκφώνηση.
- **Διπλή επαλήθευση:** το αποτέλεσμα της GPU συγκρίνεται κατευθείαν με
